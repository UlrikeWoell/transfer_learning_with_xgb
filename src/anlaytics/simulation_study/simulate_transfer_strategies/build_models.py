...
import src.xgboost_training.strategy_executor as ts

strategies = [
    ts.TgtOnlyStrategyExecutor,
    ts.SrcOnlyStrategyExecutor,
    ts.CombinationStrategyExecutor,
    ts.FreezingStrategy,
    ...
    for ep in experiment_paths:
        ...
        for ro in finetuning_options:
            ...
            if not filename in os.listdir(ep):
                print(ep, filename)
                st = ts.FinetuningStrategy(
                    path=ep, augment=augment, prune=prune, reweight=reweigth
                )
                st.execute()
