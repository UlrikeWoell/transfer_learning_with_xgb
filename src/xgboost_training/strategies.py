...
class ContinuationStrategy(Strategy):
    def execute(self) -> None:
        ...
        try:
            data = self.load_data()
            tuning_results = tuner.tune_continuation_model(
    ...
    def result_path(self) -> str:
        return f"{self.path}/continuation_result.json"


class ContinuationStrategyES(Strategy):
    def execute(self) -> None:
        tuner = self.setup_tuner()
        try:
            data = self.load_data()
            tuning_results = tuner.tune_continuation_model_early_stopping(
                X_train_1=data["src_train_X"],
                y_train_1=data["src_train_y"],
                X_train_2=data["tgt_train_X"],
                y_train_2=data["tgt_train_y"],
    ...
    def result_path(self) -> str:
        return f"{self.path}/continuation_es_result.json"


class RevisionStrategy(Strategy):
    def __init__(
        self, path: str, augment: bool = True, prune: bool = True, reweigth: bool = True
    ) -> None:
        self.path = path
        self.augment = augment
        self.prune = prune
        self.reweight = reweigth

    def execute(self) -> None:
        tuner = self.setup_tuner()
        try:
            data = self.load_data()
            tuning_results = tuner.tune_revision_model(
    ...
    def result_path(self) -> str:
        return f"{self.path}/revision_result_a{str(self.augment)}_p{str(self.prune)}_r{str(self.reweight)}.json"
